...
export default class InteractionCreate {
  ...
  async run(message: Message) {
    ...
    let gRes = await this.client.db.guild.findOne({ guildID: message.guildID })
    ...
    if (!gRes) {
      ...
      gRes = await this.client.db.guild.findOne({ guildID: message.guildID });
    };
    ...
    if (message.content.startsWith(`<@${this.client.user.id}>`) || message.content.startsWith(`<@!${this.client.user.id}>`)) {
      let embed1 = new this.client.embed()
      ...
      return message.channel.createMessage({ embeds: [embed1] })
    };
    ...
    if (command) {
      if (message.author.id !== "733963304610824252") {
        let verifmanu = await this.client.db.bot.findOne({ botID: this.client.user.id })
        if (verifmanu) {
