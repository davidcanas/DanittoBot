  ...
  async run(interaction: Interaction) {
    ...
    if ((interaction instanceof ComponentInteraction)) {
      if (interaction.data.custom_id === "delmsgeval") { 
        ...
        interaction.channel.messages.get(interaction.message.id).edit({content: "🔒 Eval Fechado!", embeds: [], components: []})
      }
      if (interaction.data.custom_id === "delmsgshell") { 
        if (interaction.member?.id !== "733963304610824252") return
        interaction.channel.messages.get(interaction.message.id).edit({content: "🔒 Shell Fechado!", embeds: [], components: []})
      }
    }
